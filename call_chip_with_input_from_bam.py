...
def tag2region(bam_info,args):
    ...
    if 'mm10' == bam_info['genome_build']:
        ...
        genome='hs'
    if bam_info['readtype'] == 'se':
        cmds.append(["macs2 callpeak -t",bam,"-c",bam_info['control'],"-f BAM","-n",prefix,"-g",genome,
                 "-p",str(args.pval),"--shift",str(shiftsize),'--extsize',str(smooth_window),'--nomodel',
                 '--keep-dup all','--call-summits'])
    elif bam_info['readtype'] == 'pe':
        cmds.append(["macs2 callpeak -t",bam,"-c",bam_info['control'],"-f BAMPE","-n",prefix,"-g",genome,
                 "-p",str(args.pval),"--shift",str(shiftsize),'--extsize',str(smooth_window),'--nomodel',
                 '--keep-dup all','--call-summits'])
...
def process_csv(args):
    ...
    for line in open(args.experiment_template):
        ...
        expts[exptname].append({'experiment':exptname,
                                ...
                                'readtype':data[header.index('pe/se')],
                                'control':data[header.index('ctrl')]})
    return expts
...
#Steps
    ...
    for expt in expt_data.keys():
        ...
        with open(expt+'/call_chip.sh','w') as f:
            for cmd in cmds:
                f.write(' '.join(cmd)+'\n')
        subprocess.run(["qsub -m e -M jhammelm@mit.edu -pe slots.pe "+str(opts.nthreads)+" -v PATH=$PATH -wd $PWD -N call_chip-"+expt+" ./"+expt+"/call_chip.sh"],shell=True)
